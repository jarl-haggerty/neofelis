...
def storeQuery(query, name):
  queryLock.aquire()
  
  fileName = "queries/query" + str(queryNumber) + ".fas"
  queryNumber += 1
...
class Pipeline():
  def __init__(self, blastLocation, genemarkLocation, transtermLocation, database, eValue, matrix, minLength, scaffoldingDistance, ldfCutoff, queries, swing = False, email = ""):
    ...
    self.blastLocation = blastLocation
    self.genemarkLocation = genemarkLocation
    self.transtermLocation = transtermLocation
    self.database = database
    self.eValue = eValue
    self.matrix = matrix
    self.minLength = minLength
    self.scaffoldingDistance = scaffoldingDistance
    self.promoterScoreCutoff = promoterScoreCutoff
  ...
  def initializeDisplay(queries, swing):
    ...
    if swing:
      ...
      self.globalProgress = JProgressBar(0, numJobs)
      self.currentLabel = JLabel(max(messages, key = len))
      self.currentProgress = JProgressBar(0, len(messages))
      self.doneButton = JButton(DoneAction())
